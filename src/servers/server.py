"""
mini_flask.py — Un micro-framework style Flask, 100% standard library

Fonctionnalités :
- Routing avec décorateur @app.route("/path", methods=["GET","POST"]) et paramètres dynamiques /users/<id>
- Request/Response objets (query, form, json, headers, files basiques)
- Helpers de réponse: text(), html(), json(), redirect(), file()
- Middleware (before_request/after_request via app.use(middleware))
- Gestion d'erreurs personnalisée @app.error(404), @app.error(500)
- Templates très simples (string.Template) via app.render_template()
- Static files: app.static("/static", dir="static")
- Serveur WSGI (wsgiref) + possibilité d'utiliser gunicorn/uvicorn

Limitations :
- Pas de reloader auto (on peut lancer avec python -m "mini_flask" --reload si vous ajoutez watchdog)
- Parser de fichiers upload très simple
"""
...
class Route:
    ...
    def _compile(self, pattern):
        # Convertit "/users/<id>" en regex ^/users/(?P<id>[^/]+)$
...
class Creator:
    def __init__(self, name=__name__, template_dir="templates"):
        ...
        self.static_mounts = []  # list of (url_prefix, dir)
    ...
    # Middleware: fn(req, next) -> Response
    ...
    def __call__(self, environ, start_response):
        ...
        # Static serving
        for prefix, folder in self.static_mounts:
        ...
        for route in self.routes:
            ...
            if params is not None and req.method in route.methods:
                ...
                try:
                    ...
                    # 500 handler
                    handler500 = self.error_handlers.get(500)
                    if handler500:
                        try:
        ...
        # Not found
        handler404 = self.error_handlers.get(404)
        if handler404:
            resp = handler404(req)
        else:
    ...
    def _ensure_response(self, resp):
        if isinstance(resp, Response):
            return resp
        # allow (body, status, headers) tuples or plain strings/dicts
    ...
    def _status_text(code):
        mapping = {200:"OK",201:"Created",204:"No Content",302:"Found",400:"Bad Request",401:"Unauthorized",403:"Forbidden",404:"Not Found",500:"Internal Server Error"}
...
# -----------------------------
# Exemple d'application utilisant le micro-framework
# -----------------------------
if __name__ == "__main__":
    app = Creator(__name__)

    # Middleware de log
    def logger(req, nxt):
        print(f"--> {req.method} {req.path} ?{req.query_string}")
        resp = nxt(req)
        print(f"<-- {resp.status}")
        return resp
    ...
    def home(req: Request):
        ...
        <html><head><meta charset='utf-8'><title>Creator</title></head>
        <body>
        <h1>Bienvenue 👋</h1>
        <p>Essayez : <a href='/hello?name=Khalil'>/hello?name=Khalil</a>, <a href='/users/42'>/users/42</a></p>
        <p>Assets statiques: <img src='/static/python.png' width='80'></p>
    ...
    def user(req: Request):
        return Response.text(f"User id = {req.params['id']}")

    @app.route("/echo", methods=["POST"])
    def echo(req: Request):
        return Response.json({"you_sent": req.form.get("msg", "")})

    @app.error(404)
    def not_found(req: Request):
        return Response.html("<h1>404</h1><p>Cette page n'existe pas.</p>", 404)

    @app.error(500)
    def server_error(req: Request, err: Exception):
