...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0">Liste des tarifs</h1>
            <p class="text-muted mb-0">Retrouvez ci-dessous tous les tarifs disponibles dans l'application.</p>
        </div>
                ...
                <tbody>
                    @foreach ($tarifs as $tarif)
                        <tr>
                            ...
                            <td>{{ $tarif->prix->format() }}</td>
                            <td>{{ $tarif->duree }}</td>
                            <td>{{ $tarif->reduction }}</td>
                            <td>
                                @component('components.tags', ['badge' => $tarif->statut])
                                @endcomponent
                            </td>
                            ...
                            <td class="no-select">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('tarifs.show', $tarif->id) }}"><i
                                                    class="bi bi-eye me-2"></i>Consulter</a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="{{ route('tarifs.edit', $tarif->id) }}"><i
                                        ...
                                        <li>
                                            <a class="dropdown-item text-danger" href="#delete-tarif-{{ $tarif->id }}"
                                                data-bs-toggle="modal"
                                                data-bs-target="#delete-tarif-{{ $tarif->id }}"><i
    ...
    @foreach ($tarifs as $tarif)
        <div class="modal fade" id="delete-tarif-{{ $tarif->id }}" tabindex="-1"
            aria-labelledby="tarif-{{ $tarif->id }}-label" aria-hidden="true">
            @component('components.modals.delete', ['action' => route('tarifs.destroy', $tarif->id)])
            @endcomponent
        </div>
    @endforeach
