...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0">Mes categories</h1>
            <p class="text-muted mb-0">Retrouvez ci-dessous tous vos categories disponibles dans l'application.</p>
        </div>
            ...
            <table class="datatables table table-striped table-hover align-middle w-100">
                ...
                <tbody>
                    @foreach ($categories as $categorie)
                        <tr>
                            <td><img src="{{ asset('assets/images/' . $categorie->image()) }}" alt="image"
                                    class="img-sm img-square" /></td>
                            <td>{{ $categorie->designation }}</td>
                            <td>{{ $categorie->Categorie->designation ?? '' }}</td>
                            <td>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow">
                                        ...
                                        <li>
                                            <a class="dropdown-item text-danger"
                                                href="#delete-categorie-{{ $categorie->id }}" data-bs-toggle="modal"
                                                data-bs-target="#delete-categorie-{{ $categorie->id }}"><i
    ...
    @foreach ($categories as $categorie)
        <div class="modal fade" id="delete-categorie-{{ $categorie->id }}" tabindex="-1"
            aria-labelledby="categorie-{{ $categorie->id }}-label" aria-hidden="true">
            @component('components.modals.delete', ['action' => route('categories.destroy', $categorie->id)])
            @endcomponent
        </div>
    @endforeach
