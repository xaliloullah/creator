...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Retrouvez ci-dessous tous les utilisateurs disponibles dans l'application.</p>
        </div>
            ...
            <table class="datatables table table-striped table-hover align-middle w-100">
                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr>
                            <td><img src="{{ asset('assets/images/' . $user->image()) }}" alt="profil"
                                    class="img-xs img-square rounded-circle profil mx-auto me-2" />{{ $user->prenom }}
                                {{ $user->nom }}</td>
                            <td>
                                @component('components.tags', ['badge' => $user->email_statut])
                                @endcomponent
                                {{ $user->email }}
                            </td>
                            ...
                            <td class="no-select">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow">
                                        ...
                                        <li>
                                            <a class="dropdown-item text-danger" href="#delete-user-{{ $user->id }}"
    ...
    @foreach ($users as $user)
        <div class="modal fade" id="delete-user-{{ $user->id }}" tabindex="-1"
            ...
            @component('components.modals.delete', ['action' => route('users.destroy', $user->id)])
            @endcomponent
        </div>
    @endforeach
