...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Retrouvez ci-dessous tous les clients disponibles dans l'application.</p>
        </div>
            ...
            <table class="datatables table table-striped table-hover align-middle w-100">
                ...
                <tbody>
                    @foreach ($clients as $client)
                        <tr>
                            <td><img src="{{ asset('assets/images/' . $client->image()) }}" alt="profil"
                                    class="img-xs img-square rounded-circle profil mx-auto me-2" />{{ $client->prenom }}
                                {{ $client->nom }}</td>
                            <td>
                                @component('components.tags', ['badge' => $client->email_statut])
                                @endcomponent
                                {{ $client->email }}
                            </td>
                            ...
                            <td class="no-select">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow">
                                        ...
                                        <li>
                                            <a class="dropdown-item text-danger" href="#delete-client-{{ $client->id }}"
                                                data-bs-toggle="modal"
                                                data-bs-target="#delete-client-{{ $client->id }}"><i
    ...
    @foreach ($clients as $client)
        <div class="modal fade" id="delete-client-{{ $client->id }}" tabindex="-1"
            ...
            @component('components.modals.delete', ['action' => route('clients.destroy', $client->id)])
            @endcomponent
        </div>
    @endforeach
