...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0">Mes plats</h1>
            <p class="text-muted mb-0">Retrouvez ci-dessous tous vos plats disponibles dans l'application.</p>
        </div>
            ...
            <table class="datatables table table-striped table-hover align-middle w-100">
                ...
                <tbody>
                    @foreach ($plats as $plat)
                        <tr> 
                            <td><img src="{{ asset('assets/images/' . $plat->image) }}" alt="image"
                                class="img-xs img-square rounded me-3" />{{ $plat->designation }}</td>
                            <td> <a
                                    href="{{ route('menus.show', $plat->Menu->id) }}">{{ $plat->Menu->designation ?? '' }}</a>
                            </td>
                            <td>{{ $plat->prix }}</td>
                            <td>{{ $plat->duree }}</td>
                            <td>{{ $plat->reduction }}</td>
                            <td>
                            ...
                            <td class="no-select">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('plats.show', $plat->id) }}"><i
                                                    class="bi bi-eye me-2"></i>Consulter</a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="{{ route('plats.edit', $plat->id) }}"><i
                                        ...
                                        <li>
                                            <a class="dropdown-item text-danger" href="#logout" data-bs-toggle="modal"
                                                data-bs-target="#delete-plat-{{ $plat->id }}"><i
    ...
    @foreach ($plats as $plat)
        <div class="modal fade" id="delete-plat-{{ $plat->id }}" tabindex="-1"
            aria-labelledby="plat-{{ $plat->id }}-label" aria-hidden="true">
            @component('components.modal-delete', ['action' => route('plats.destroy', $plat->id)])
            @endcomponent
        </div>
    @endforeach
